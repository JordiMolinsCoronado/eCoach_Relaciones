...
def main() -> None:
    ...
    app.add_handler(CallbackQueryHandler(handle_compare_current_portfolio_button, pattern=f"^{COMPARE_CURRENT_PORTFOLIO_CALLBACK}$"))
    app.add_handler(CallbackQueryHandler(handle_define_concrete_alternative_button, pattern=f"^{DEFINE_CONCRETE_ALTERNATIVE_CALLBACK}$"))
    app.add_handler(CallbackQueryHandler(handle_prepare_provisional_alternative_button, pattern=f"^{PREPARE_PROVISIONAL_ALTERNATIVE_CALLBACK}$"))
    app.add_handler(CallbackQueryHandler(handle_define_initial_alternative_button, pattern=f"^{DEFINE_INITIAL_ALTERNATIVE_CALLBACK}$"))
    ...
    app.add_handler(CallbackQueryHandler(handle_keep_similar_risk_button, pattern=f"^{KEEP_SIMILAR_RISK_CALLBACK}$"))
    app.add_handler(CallbackQueryHandler(handle_reduce_risk_button, pattern=f"^{REDUCE_RISK_CALLBACK}$"))
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND & filters.Regex(PUBLIC_ENRICHMENT_RE), handle_public_enrichment_command), group=0)
