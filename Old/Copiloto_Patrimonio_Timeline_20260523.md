...
## v24c â€” IntegraciÃ³n DeepSeek V4 Pro API

**Objetivo:** aÃ±adir DeepSeek V4 Pro como segundo frontier model importante del sistema, complementando Gemini Flash-Lite.

MotivaciÃ³n estratÃ©gica:

```text
- razonamiento mÃ¡s fuerte a coste moderado;
- redundancia multi-provider;
- comparaciÃ³n de calidad entre modelos;
- posibilidad de enrutar tareas segÃºn coste/inteligencia;
- preparar arquitectura futura de escalado inteligente.
```

Arquitectura conceptual:

```text
LLM local/confidencial
â†’ decide si hace falta frontier model
â†’ Gemini Flash-Lite para tareas baratas/simples
â†’ DeepSeek V4 Pro para razonamiento mÃ¡s potente
â†’ modelos premium solo cuando realmente aporten valor
```

Casos de uso esperados para DeepSeek V4 Pro:

```text
- razonamiento patrimonial complejo;
- sÃ­ntesis de documentos;
- comparaciones sofisticadas;
- anÃ¡lisis mÃ¡s largos;
- generaciÃ³n de respuestas de mayor calidad;
- pruebas comparativas de frontier models.
```

Aprendizaje estratÃ©gico esperado:

```text
La ventaja competitiva probablemente no estarÃ¡ en â€œtener acceso a un modelo frontierâ€,
sino en:
- privacidad;
- orquestaciÃ³n;
- memoria;
...
- calidad del flujo cognitivo.
```

**Estado:** prÃ³ximo paso inmediato o casi inmediato.
...
## v40b â€” Salida dual del orquestador: respuesta privada + triggers proactivos

**Objetivo:** permitir que el orquestador, ademÃ¡s de responder al cliente, proponga seguimientos futuros de baja sensibilidad que Prevengen sÃ­ puede guardar para ser proactivo.

Idea central:

```text
No todo lo que produce el orquestador debe tener el mismo destino.

Parte privada:
â†’ se envÃ­a solo al cliente;
â†’ actualiza archivos vivos privados;
â†’ puede contener contexto sensible.

Parte operativa mÃ­nima:
â†’ se envÃ­a tambiÃ©n a Prevengen;
â†’ contiene solo triggers seguros de seguimiento;
â†’ sirve para proactividad futura.
```

El orquestador debe devolver una salida estructurada con dos capas:

```json
{
  "respuesta_cliente": "...",
  "actualizaciones_privadas": {
    "quien_soy": "...",
    "que_quiero": "...",
    "que_tengo_que_hacer": "..."
  },
  "followup_triggers": [
    {
      "type": "roboadvisor_review",
      "date": "2026-08-23",
      "message_template": "Puede ser buen momento para revisar si tu estrategia de inversiÃ³n sigue encajando contigo.",
      "sensitivity": "low",
...
```text
La respuesta completa y las actualizaciones privadas van al cliente.
Prevengen solo guarda los triggers mÃ­nimos aprobados.
```
...
```text
Privado:
â€œEl cliente decidiÃ³ invertir 180.000 â‚¬ en ROBOADVISOR_1 y teme volatilidad.â€

Trigger visible para Prevengen:
â€œ2026-08-23 â€” roboadvisor_review â€” revisar si la estrategia sigue encajando.â€
```

La frase clave:

> Prevengen no necesita conocer la wiki para ser proactivo.  
> Solo necesita guardar triggers mÃ­nimos aprobados.

**Estado:** futuro prÃ³ximo.

---

## v40c â€” Skill interna de follow-ups proactivos

**Objetivo:** crear un mÃ³dulo Markdown que instruya al orquestador para detectar cuÃ¡ndo una interacciÃ³n deberÃ­a generar seguimientos futuros.

Archivo sugerido:

```text
prompts/proactive_followup_skill.md
```

Contenido conceptual:

```text
Cuando el cliente tome una decisiÃ³n importante, propÃ³n posibles checkpoints futuros.

Tipos de follow-up:
- revisiÃ³n de roboadvisor;
- revisiÃ³n de propuesta bancaria;
- revisiÃ³n fiscal antes de final de aÃ±o;
- revisiÃ³n de liquidez;
- revisiÃ³n de tolerancia al riesgo;
- revisiÃ³n tras cambio familiar;
- revisiÃ³n tras cambio profesional;
- revisiÃ³n de costes o comisiones;
- revisiÃ³n de documentaciÃ³n pendiente.
```
...
```text
1. Memoria privada:
   puede ser detallada, pero no debe salir a Prevengen si el modo privado lo impide.

2. Trigger externo:
   debe ser genÃ©rico, mÃ­nimo y de baja sensibilidad.
```
...
```json
{
  ...
  "date": "2026-09-01",
  "message_template": "Puede ser buen momento para revisar si tu situaciÃ³n patrimonial sigue alineada con tus objetivos.",
  "sensitivity": "low"
}
```

Ejemplo de trigger incorrecto:

```json
{
  "type": "review_800000_euros_caixabank_indexa",
  "message_template": "Revisar los 800.000 â‚¬ de Caixabank e Indexa porque el cliente estÃ¡ ansioso por jubilarse."
}
```
...
## v40d â€” Archivo de triggers proactivos

**Objetivo:** crear un archivo operativo donde el sistema pueda guardar seguimientos futuros seguros para Prevengen.

Archivo sugerido:

```text
ClientData/Cliente1/followup_triggers.md
```

O, en formato estructurado:
...
```json
{
  ...
  "source_interaction_id": "interaction_abc"
}
```
...
```text
Este archivo no debe contener importes exactos, bancos concretos, nombres familiares, detalles mÃ©dicos, ni motivos privados.
```

**Para quÃ© sirve:** permite que el bot sea proactivo sin convertir la memoria privada en un dossier visible para Prevengen.

**Estado:** futuro prÃ³ximo.

---

## v40e â€” Scheduler diario de follow-ups

**Objetivo:** aÃ±adir un proceso que revise cada dÃ­a si hay triggers pendientes y envÃ­e mensajes por Telegram cuando corresponda.

Flujo:

```text
1. Scheduler se ejecuta cada dÃ­a.
2. Lee followup_triggers.
3. Busca triggers con date <= hoy y status = pending.
4. EnvÃ­a mensaje seguro al cliente por Telegram.
5. Marca el trigger como sent.
6. Registra el evento en logs.
```

Mensaje tipo:

```text
Puede ser buen momento para revisar si tu estrategia patrimonial sigue encajando contigo.

Si quieres, abre una revisiÃ³n privada y repasamos si algo ha cambiado.
```

La parte pÃºblica solo despierta la conversaciÃ³n.

El anÃ¡lisis real ocurre despuÃ©s:

```text
Usuario responde
â†’ capa privada / orquestador lee contexto permitido
â†’ se decide si hace falta LLM externo
â†’ se actualizan archivos vivos
```
...
## v40f â€” Proactividad en Telegram

**Objetivo:** que Copiloto Relaciones pueda iniciar conversaciones Ãºtiles con el cliente, no solo responder cuando el cliente escribe.

Casos iniciales:

```text
- revisiÃ³n de una decisiÃ³n patrimonial tomada hace 3 meses;
- recordatorio de pedir documentaciÃ³n pendiente;
- revisiÃ³n fiscal antes de final de aÃ±o;
- seguimiento tras conversaciÃ³n con banco o asesor;
- comprobaciÃ³n de si una propuesta sigue encajando;
- revisiÃ³n periÃ³dica de objetivos.
```
...
```text
- mensajes breves;
- tono sobrio y no invasivo;
- no incluir datos sensibles en Telegram;
- no mencionar importes, bancos ni detalles privados;
- permitir desactivar follow-ups;
- permitir posponer;
- permitir marcar como hecho.
```
...
Posponer
Marcar como hecho
Desactivar este tipo de avisos
```

Mensaje ejemplo:

```text
Puede ser buen momento para revisar si tu decisiÃ³n patrimonial sigue alineada con tus objetivos.

No hace falta decidir nada ahora. Podemos empezar por comprobar si ha cambiado algo importante.
```

**Para quÃ© sirve:** convierte el producto en acompaÃ±amiento real en el tiempo.

**Estado:** muy importante para producto.

---

## v40g â€” Registro de proactividad y consentimiento

**Objetivo:** mantener trazabilidad de quÃ© avisos se han creado, enviado, pospuesto o desactivado.
...
```text
- fecha de creaciÃ³n del trigger;
- origen;
- tipo de seguimiento;
- fecha programada;
- mensaje enviado;
...
- si se pospuso;
- si se desactivÃ³;
- si abriÃ³ revisiÃ³n privada.
```

Regla de confianza:

```text
El cliente debe poder ver y borrar los follow-ups pendientes.
```
...
â†’ una llamada fuerte al orquestador Gemini / DeepSeek segÃºn coste y calidad
â†’ actualizaciÃ³n de archivos vivos
â†’ respuesta final al cliente
â†’ creaciÃ³n opcional de triggers mÃ­nimos de follow-up
â†’ log
â†’ scheduler proactivo de Telegram
```
