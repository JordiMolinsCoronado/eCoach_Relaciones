...
- suggest questions for bank or advisor;
- remind about pending documents;
- review whether a decision still fits;
- detect contradiction or premature decision;
- suggest one small next action;
- create a follow-up after an important meeting or decision.
...
- send generic reminders with no clear value;
...
async def show_agent_config(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ensure_client_files()

    global_config = load_agent_config_files()
    client_config = load_client_agent_config()

    text = (
        "# Global AgentConfig\n\n"
        f"{global_config or 'No global AgentConfig files found.'}\n\n"
        "---\n\n"
        "# Client agent_config.md\n\n"
        f"{client_config or 'No client agent_config.md found.'}"
    )
...
def main() -> None:
    ...
    app.add_handler(CommandHandler("agent_config", show_agent_config))
